...
use crate::migrator::m20230916_000003_create_video_upload_table::VideoUpload;
...
impl MigrationTrait for Migration {
    #[instrument(skip(manager, self), name = "inserting_sample_data")]
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        debug!("inserting sample data");
        for values in Values::get_users() {
            Self::add_user(manager, values).await?;
            debug!("inserted user");
        }
        debug!("inserted user");
        for values in Values::get_videos() {
            Self::add_video(manager, values).await?;
            debug!("inserted video");
        }
        for values in Values::get_video_uploads() {
            Migration::add_video_upload(manager, values).await?;
    ...
    #[instrument(skip(manager, self), name = "delete_sample_data")]
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        info!("deleting sample data");
        for values in Values::get_video_uploads() {
            Migration::remove_video_upload(manager, values).await?;
            debug!("removed video upload");
        }
        for values in Values::get_videos() {
            Migration::remove_video(manager, values).await?;
            debug!("removed video");
        }
        for values in Values::get_users() {
            Migration::remove_user(manager, values).await?;
            debug!("removed user");
        }
...
impl Migration {
    ...
    async fn remove_video_upload<'a>(
        manager: &SchemaManager<'a>,
        values: [SimpleExpr; 3],
    ) -> Result<(), DbErr> {
...
fn add_all_wheres(query: &mut DeleteStatement, values: Vec<SimpleExpr>, columns: Vec<Expr>) {
    for (val, col) in values.into_iter().zip(columns.into_iter()) {
        add_where(query, val, col);
    }
}
fn add_where(query: &mut DeleteStatement, value: SimpleExpr, column: Expr) {
    query.and_where(column.eq(value));
}
struct Values;
impl Values {
    fn get_users() -> Vec<[SimpleExpr; 10]> {
        vec![[
            3.into(),
            "123".into(),
            "test".into(),
    ...
    fn get_videos() -> Vec<[SimpleExpr; 15]> {
        vec![
            [
                1.into(),
                3.into(),
    ...
    fn get_video_uploads() -> Vec<[SimpleExpr; 3]> {
        vec![
